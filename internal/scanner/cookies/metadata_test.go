...
func TestRegister_AllChecksHaveValidMetadata(t *testing.T) {
	...
	for _, c := range r.All() {
		...
		if !strings.HasPrefix(c.ID(), "COOKIE-") {
			t.Errorf("%s does not start with COOKIE-", c.ID())
		}
		if c.Family() != checks.FamilyCookies {
			t.Errorf("%s family = %s, want cookies", c.ID(), c.Family())
		}
