...
func TestRegister_AllChecksHaveValidMetadata(t *testing.T) {
	...
	for _, c := range r.All() {
		...
		if !strings.HasPrefix(c.ID(), "WELLKNOWN-") {
			t.Errorf("%s does not start with WELLKNOWN-", c.ID())
		}
		if c.Family() != checks.FamilyWellKnown {
			t.Errorf("%s family = %s, want wellknown", c.ID(), c.Family())
		}
