...
├── cmd/
│   ├── websec0/                    # binaire serveur (HTTP API + frontend)
│   │   └── main.go
│   └── websec0-cli/                # binaire CLI standalone
│       └── main.go
├── internal/
│   ├── api/
...
Binaire séparé `websec0-cli` (peut aussi être invoqué via `websec0 scan` quand le serveur est utilisé en mode local).
...
├── scripts/
│   ├── scan.sh                 # wrapper curl/websec0-cli
...
curl -sSL https://websec0.example/install.sh | sh

# Script alternatif manuel (vérification cosign + SHA256)
# Documenté dans docs/self-hosting.md

# go install
go install github.com/your-org/websec0/cmd/websec0@latest

# Homebrew (macOS / Linux)
brew install your-org/tap/websec0

# Docker
docker run --rm -p 8080:8080 ghcr.io/your-org/websec0:latest

# docker-compose pour auto-hébergement
# (template fourni dans deploy/docker/docker-compose.yml)

# Binaire pré-compilé
# Téléchargement depuis https://github.com/your-org/websec0/releases
```
...
- **MCP server** `websec0-mcp` exposant tools structurés pour Claude/Cursor
